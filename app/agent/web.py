...
class WebReader:
    ...
    async def read(self, url, domains):
        ...
        final, status, mime, text = await self.request(url, domains)
        # A redirected page must also be permitted by the destination robots policy.
        if final != url:
            # Conservatively reject document redirects; robots redirects remain bounded.
            raise ToolFailure('document_redirect_rejected')
        if status != 200:
