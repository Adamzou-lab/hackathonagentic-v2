...
Pour une veille courte (budget de 10 actions ou moins), l’objectif est fixé à deux constats sauvegardés. Une fois cet objectif atteint, seuls la clôture et le refus explicite restent proposés au modèle : aucune nouvelle lecture ou sauvegarde en double. Le nombre d’actions reste un plafond, et atteindre la limite avant l’objectif reste signalé comme une synthèse partielle.
